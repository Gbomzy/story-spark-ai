  ...
  head: () => ({
    meta: [
      { title: "Story Music Engine — StorySpark AI" },
      {
        name: "description",
        content:
          "Analyze a story and design an intelligent music plan: scene-by-scene background music, a lesson-reinforcing song, and mixing controls for the Movie Composer.",
      },
    ],
  }),
  component: SongsPage,
      ...
      {!project ? (
        <Card className="glass rounded-3xl p-8 text-center shadow-soft">
          <p className="text-sm text-muted-foreground">
            Create a project and generate a story first.
          </p>
        ...
        <Card className="glass rounded-3xl p-8 text-center shadow-soft">
          <p className="text-sm text-muted-foreground">
            This project has no story yet. Generate one before running the Music Engine.
          </p>
          <Button asChild className="mt-4 rounded-xl gradient-primary text-white shadow-glow">
    ...
    <>
      ...
      <Card className="glass rounded-3xl p-6 shadow-soft">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <div className="flex flex-wrap gap-1">
                {(["none", "intro", "middle", "ending", "multiple"] as const).map((p) => (
                  <Button
                    key={p}
                    size="sm"
                    variant={customPosition === p ? "default" : "outline"}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                <Button
                  size="sm"
                  variant={customMood === "" ? "default" : "outline"}
                  className="rounded-lg"
                  onClick={() => setCustomMood("")}
                >
                  auto
                </Button>
                {BGM_MOODS.map((m) => (
                  <Button
                    key={m}
                    size="sm"
                    variant={customMood === m ? "default" : "outline"}
                    className="rounded-lg"
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
          <p className="text-[11px] text-muted-foreground">
            The engine reads the story, extracts theme + lesson + emotional arc, then recommends
            background music per scene and decides if a song helps.
          </p>
          <Button
            onClick={() => mut.mutate()}
          ...
          >
            {mut.isPending ? (
              ...
              <>
                ...
                {plan ? "Re-analyze story" : "Analyze story"}
              </>
            )}
          </Button>
        </div>
        ...
        {legacyLyrics && !plan ? (
          <div className="mt-4 rounded-xl border border-dashed border-border bg-muted/30 p-3 text-[11px] text-muted-foreground">
            Legacy lyrics from the story generator are stored on this project. Run the analyzer to
            upgrade to a full music plan.
          </div>
        ) : null}
      </Card>
      ...
      {plan ? (
        <>
            ...
            <div className="mt-4 grid gap-3 rounded-2xl border border-border bg-card/60 p-4 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-0.5 text-sm font-semibold">
                  {plan.recommendation.backgroundStyle || "—"}
                </p>
              </div>
              <div>
                <p className="text-[10px] uppercase tracking-widest text-muted-foreground">
          ...
          {plan.song ? (
            <Card className="glass rounded-3xl p-6 shadow-soft">
              ...
              <p className="mt-3 text-[11px] text-muted-foreground">
                <strong>Reinforces lesson:</strong> {plan.song.reinforcesLesson}
              </p>
              <div className="mt-4 flex flex-wrap gap-2">
                <Button variant="outline" className="rounded-xl" onClick={copySong}>
                  <Copy className="mr-1.5 h-4 w-4" /> Copy
                </Button>
                <Button variant="outline" className="rounded-xl" onClick={downloadSong}>
              ...
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-border bg-muted/40 p-3 text-[11px] text-muted-foreground">
                ...
                <span>
                  Music synthesis is currently unavailable under the configured Qwen Cloud
                  capabilities. Lyrics remain available for export.
                </span>
              </div>
            </Card>
          ) : null}
          ...
          <Card className="glass rounded-3xl p-6 shadow-soft">
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <Button
                ...
                onClick={() => saveScenesMut.mutate()}
                disabled={saveScenesMut.isPending}
              >
                {saveScenesMut.isPending ? (
            ...
            <p className="mt-3 text-[11px] text-muted-foreground">
              Music volume sits under narration. Automatic ducking (below) lowers it further while
          ...
          <EndingCreditsCard
            ec={studio.endingCredits}
            onChange={(ec) => patchStudio({ endingCredits: ec })}
          />
        </>
      ) : null}
    </>
...
function mergeScenes(
  current: AudioStudioScene[],
  planScenes: StoryMusicPlan["scenes"],
): AudioStudioScene[] {
...
function SceneRow({
  ...
  scene: AudioStudioScene;
  onPatch: (p: Partial<AudioStudioScene>) => void;
}) {
  ...
  useEffect(() => {
    previewRef.current?.setUrl(scene.bgmTrackUrl);
  }, [scene.bgmTrackUrl]);
  useEffect(() => {
    previewRef.current?.setLoop(loop);
  }, [loop]);
    ...
    <div className="rounded-2xl border border-border bg-card/60 p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-1">
          {BGM_MOODS.map((m) => (
            <Button
              key={m}
              size="sm"
              variant={scene.bgmMood === m ? "default" : "outline"}
              className="rounded-lg text-[11px]"
              onClick={() => onPatch({ bgmMood: m })}
            >
              {m}
            </Button>
          ))}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <VolumeSlider
          ...
          onChange={(v) => onPatch({ musicVolume: v })}
        />
        <VolumeSlider
          label="Narration volume"
          value={scene.narrationVolume}
          onChange={(v) => onPatch({ narrationVolume: v })}
        />
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {scene.sfx.map((s) => (
            <div
              key={s.id}
              className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-card/40 p-2"
            >
              ...
              <div className="flex items-center gap-1 w-40">
                ...
                <input
                  ...
                  onChange={(e) => patchSfx(s.id, { volume: Number(e.target.value) / 100 })}
                  className="flex-1"
                />
...
function DuckingCard({
  ...
  ducking: AudioStudioState["ducking"];
  onChange: (d: AudioStudioState["ducking"]) => void;
}) {
  return (
    <Card className="glass rounded-3xl p-6 shadow-soft">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-1">
          <Button
            size="sm"
            variant={ducking.enabled ? "default" : "outline"}
            className="rounded-lg"
            onClick={() => onChange({ ...ducking, enabled: true })}
          >
            On
          </Button>
          <Button
            ...
            variant={!ducking.enabled ? "default" : "outline"}
            className="rounded-lg"
            onClick={() => onChange({ ...ducking, enabled: false })}
          >
            Off
          </Button>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <VolumeSlider
          label="Ducked level"
          value={ducking.duckedLevel}
          onChange={(v) => onChange({ ...ducking, duckedLevel: v })}
        />
        <VolumeSlider
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...ducking, attackMs: Number(e.target.value) })}
            className="w-full"
          />
        </div>
        <div>
          <p className="mb-1 text-[10px] uppercase tracking-widest text-muted-foreground">
            Release · {ducking.releaseMs}ms
          </p>
          <input
            type="range"
            min={50}
            ...
            onChange={(e) => onChange({ ...ducking, releaseMs: Number(e.target.value) })}
            className="w-full"
          />
        </div>
      </div>
      <p className="mt-3 text-[11px] text-muted-foreground">
        The Movie Composer analyses narration RMS in real time and ramps background music down while
        the narrator is speaking, then back up between segments.
      </p>
    </Card>
  );
}

function EndingCreditsCard({
  ec,
  onChange,
}: {
  ec: AudioStudioState["endingCredits"];
  onChange: (v: AudioStudioState["endingCredits"]) => void;
}) {
  return (
    <Card className="glass rounded-3xl p-6 shadow-soft">
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex gap-1">
          <Button
            size="sm"
            variant={ec.enabled ? "default" : "outline"}
            className="rounded-lg"
            onClick={() => onChange({ ...ec, enabled: true })}
          >
            Enable
          </Button>
          <Button
            ...
            variant={!ec.enabled ? "default" : "outline"}
            className="rounded-lg"
            onClick={() => onChange({ ...ec, enabled: false })}
          >
            Disable
          </Button>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <Input
            value={ec.trackUrl ?? ""}
            onChange={(e) => onChange({ ...ec, trackUrl: e.target.value || undefined })}
            placeholder="https://…/credits.mp3"
            className="h-8 text-xs"
          />
        </div>
        <div>
          <p className="mb-1 text-[10px] uppercase tracking-widest text-muted-foreground">
            Fade-out · {ec.fadeOutSeconds.toFixed(1)}s
          </p>
          <input
            type="range"
            min={1}
            max={10}
            step={0.5}
            value={ec.fadeOutSeconds}
            onChange={(e) => onChange({ ...ec, fadeOutSeconds: Number(e.target.value) })}
            className="w-full"
          />
        </div>
        <div className="sm:col-span-2">
          ...
          <Input
            value={ec.text ?? ""}
            onChange={(e) => onChange({ ...ec, text: e.target.value || undefined })}
            placeholder="Written &amp; produced by…"
            className="h-8 text-xs"
          />
        </div>
      </div>
...
function AnalysisStat({ label, value, span2 }: { label: string; value: string; span2?: boolean }) {
  return (
    <div
      className={`rounded-xl border border-border bg-card/60 p-3 ${span2 ? "sm:col-span-2 lg:col-span-3" : ""}`}
    >
